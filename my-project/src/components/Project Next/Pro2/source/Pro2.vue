<template>
  <header class="sticky top-0 bg-white z-50">
    <nav
      class="flex items-center justify-between w-full h-24 shadow-lg z-10 px-4 md:px-10"
    >
      <!-- Logo and Title -->
      ...
      <ul
        :class="{ 'max-h-72': isOpen, 'max-h-0': !isOpen }"
        class="md:hidden absolute top-24 left-0 w-full bg-white shadow-lg overflow-hidden transition-all ease-in-out duration-300"
      >
        <li
        ...
        >
          <span v-if="item.isButton">
            <router-link :to="item.path">
              <button
                class="block px-3 py-2 text-white bg-blue-500 text-sm md:text-base w-full text-left"
              >
                {{ item.name }}
              </button>
            </router-link>
